...
export class OpenAIService {
  ...
  constructor(apiKey: string, model?: string) {
    ...
    // Use provided model or read from environment variable, with fallback
    this.model = model || process.env.OPENAI_MODEL || "gpt-oss:120b-cloud";
  }
  ...
  createEnrichmentSchema(fields: EnrichmentField[]) {
    ...
    fields.forEach((field) => {
      confidenceProperties[`${field.name}_confidence`] = z
        .number()
        .min(0)
        .max(1);
      // Each field can have multiple sources with their own quotes
      sourceEvidenceProperties[`${field.name}_sources`] = z
        .array(
          z.object({
            url: z.string(),
            quote: z.string(),
  ...
  async extractStructuredDataOriginal(
    ...
    try {
      ...
      if (content.length > MAX_CONTENT_CHARS) {
        console.log(
          `[OPENAI] Content too long (${content.length} chars), trimming to ${MAX_CONTENT_CHARS} chars`,
        );
        trimmedContent =
...
- Look for actual company websites with real business information`,
      ...
      // Handle markdown code fences and other formatting issues
      ...
      if (cleanContent.startsWith("```")) {
        ...
        if (jsonMatch && jsonMatch[1]) {
          ...
          // If no match, try to find JSON content after first ```
          const firstBacktick = cleanContent.indexOf("```");
          const lastBacktick = cleanContent.lastIndexOf("```");
          if (
            firstBacktick !== -1 &&
            lastBacktick !== -1 &&
            ...
            cleanContent = cleanContent
              .substring(firstBacktick + 3, lastBacktick)
              .trim();
          }
        }
      }

      // Remove any remaining non-JSON content before/after
      ...
      fields.forEach((field) => {
        ...
        const sourcesWithQuotes = parsed[`${field.name}_sources`] as Array<{
          url: string;
          quote: string;
        }> | null;
        ...
        if (value !== null && value !== undefined) {
          ...
          if (
            (field.name === "employeeCount" ||
              field.displayName === "Employee Count") &&
            typeof value === "number"
          ) {
            if (value > 1000000) {
              // Likely a parsing error (e.g., "500+" interpreted as 500,000)
              console.warn(
                `Unrealistic employee count detected: ${value}. Reducing confidence.`,
              );
          ...
          if (
            (field.name === "yearFounded" ||
              field.displayName === "Year Founded") &&
            typeof value === "number"
          ) {
          ...
          if (
            (field.name === "fundingStage" ||
              field.displayName === "Funding Stage") &&
            typeof value === "string"
          ) {
        ...
        if (value !== null && value !== undefined && confidence > 0.3) {
          results[field.name] = {
            ...
            value,
            confidence,
            source: sourcesWithQuotes
              ? sourcesWithQuotes.map((s) => s.url).join(", ")
              : "structured_extraction",
            sourceContext: sourcesWithQuotes
              ? sourcesWithQuotes.map((s) => ({
                  url: s.url,
                  snippet: s.quote,
                }))
              : undefined,
      ...
      // Return empty results instead of throwing an error to allow processing to continue
      console.warn(
        "Failed to extract structured data, returning empty results",
      );
      return {};
    }
  }
  ...
  async extractStructuredDataWithCorroboration(
    ...
    try {
      console.log(
        "Starting corroborated extraction for fields:",
        fields.map((f) => f.name),
      );
...
7. Common fake URL patterns to AVOID:
   ...
   - Any URL with brackets or placeholders`,
      ...
      const { validateSnippetContainsValue } = await import(
      ...
      fields.forEach((field) => {
        ...
        const validEvidence = fieldData.evidence.filter(
          (e: {
            ...
            if (!e.exact_text || e.exact_text.trim() === "") {
              console.log(
                `[VALIDATION] Rejecting evidence with no text for ${field.name}`,
              );
              return false;
            }
            ...
            if (e.source_url) {
              try {
                ...
                if (
                  ...
                  console.log(
                    `[VALIDATION] Rejecting hallucinated URL for ${field.name}: ${e.source_url}`,
                  );
                  return false;
                }
              } catch {
                console.log(
                  `[VALIDATION] Rejecting invalid URL for ${field.name}: ${e.source_url}`,
                );
                return false;
              }
            }
            ...
            const wordCount = e.exact_text
              ? e.exact_text.split(/\s+/).length
              : 0;
            ...
            const looksLikeTitle =
              ...
              (e.exact_text.includes(" | ") ||
                ...
                (e.exact_text.match(/^[A-Z][^.!?]*$/) &&
                  e.exact_text.length < 100) ||
            ...
            if (looksLikeTitle) {
              console.log(
                `[VALIDATION] Filtering out title-like evidence for ${field.name}:`,
                e.exact_text,
              );
              return false;
            }
            ...
            const looksLikeHallucination = hallucationPatterns.some((pattern) =>
              pattern.test(e.exact_text),
            );
            if (looksLikeHallucination) {
              console.log(
                `[VALIDATION] Detected potential hallucination pattern in ${field.name}:`,
                e.exact_text,
              );
              return false;
            }
            ...
            const isValid = validateSnippetContainsValue(
              e.exact_text,
              e.value as string | number | boolean | string[],
            );
            if (!isValid) {
              console.log(
                `[VALIDATION] Filtering out evidence for ${field.name} - snippet doesn't contain value:`,
                {
                  value: e.value,
        ...
        if (validEvidence.length === 0) {
          console.log(
            `[VALIDATION] No valid evidence found for ${field.name}, skipping field`,
          );
          return;
        }
        ...
        if (validEvidence.length > 0 && fieldData.consensus_confidence > 0.2) {
          ...
          if (
            ...
            const validSourceContext = validEvidence
              .filter(
                (e: { exact_text: string }) =>
                  e.exact_text && e.exact_text.trim() !== "",
              )
            ...
            if (validSourceContext.length > 0) {
              console.log(
                `[SOURCE-CONTEXT] For ${field.name}, keeping ${validSourceContext.length} sources:`,
                validSourceContext.map((sc) => ({
                  url: sc.url,
                  snippet: sc.snippet.substring(0, 50) + "...",
                })),
              );
            ...
            const allSnippetsAreTitles = validSourceContext.every((sc) => {
              ...
              const hasTitle =
                text.includes(" | ") ||
                (text.includes(" - ") && !hasSentenceEnding);
              return (
                hasTitle ||
                (!hasSentenceEnding && text.split(/\s+/).length < 15)
              );
            });

            if (allSnippetsAreTitles) {
              console.log(
                `[VALIDATION] All snippets for ${field.name} look like titles, clearing source context`,
              );
            ...
            if (validSourceContext.length > 0 || consensusValue !== null) {
              ...
              if (validSourceContext.length === 0) {
                console.log(
                  `[CONFIDENCE] No valid evidence for ${field.name}, reducing confidence from ${adjustedConfidence} to 0.1`,
                );
              ...
              if (
                field.name.toLowerCase().includes("tech") ||
                field.name.toLowerCase().includes("stack")
              ) {
                // Validate GitHub URLs in evidence
                if (enrichmentResult.corroboration) {
                  enrichmentResult.corroboration.evidence =
                    enrichmentResult.corroboration.evidence.filter((e) => {
                      if (e.source_url && e.source_url.includes("github.com")) {
                        // Check if this GitHub URL was provided in the context
                        const validGithubUrls = Array.isArray(
                          context.validGithubUrls,
                        )
                          ? (context.validGithubUrls as string[])
                          : [];
                        const isValid = validGithubUrls.includes(e.source_url);
                        if (!isValid) {
                          console.log(
                            `[VALIDATION] Removing hallucinated GitHub URL from evidence: ${e.source_url}`,
                          );
                          return false;
                        }
                      }
                ...
                if (Array.isArray(enrichmentResult.value)) {
                  ...
                  enrichmentResult.value = enrichmentResult.value.filter(
                    (tech) => {
                  ...
                  if (enrichmentResult.value.length === 0) {
                    console.log(
                      `[VALIDATION] No valid technologies found for ${field.name}, skipping field`,
                    );
                    return;
                  }
                }
              ...
              if (fieldData.evidence.length > 0) {
                console.log(
                  `Field ${field.name} has ${fieldData.evidence.length} evidence items, sources_agree: ${fieldData.sources_agree}`,
                );
              }
...
- Be specific with your answer, citing actual data values`,
      ...
      const isGenericAnswer =
        ...
        (answer.toLowerCase().includes("table contains") &&
          answer.toLowerCase().includes("rows")) ||
        (answer.toLowerCase().includes("total") &&
          answer.toLowerCase().includes("rows") &&
          answer.split(" ").length < 15);

      if (isGenericAnswer) {
        console.log(
          "[OPENAI] Table data check: NOT_FOUND (generic or not found)",
        );
        return { found: false };
      }

      console.log(
        "[OPENAI] Table data check: FOUND -",
  ...
  async selectBestSource(
    ...
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a source evaluator. Select the best source to answer the question. Return JSON with {index: number}.",
          },
          {
            role: "user",
      ...
      const result = JSON.parse(
        response.choices[0].message.content || '{"index": 0}',
      );
