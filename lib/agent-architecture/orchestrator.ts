...
export class AgentOrchestrator {
  ...
  async enrichRow(
    ...
    try {
      // Initialize context and enrichments early to avoid reference errors
      ...
      const emailContext = context.emailContext;
      // Step 2: If cached company data is provided, use it to pre-populate discovered data
      if (cachedCompanyData) {
        console.log(
          `[Orchestrator] Using cached company data for: ${emailContext.domain}`,
        );
        const prePopulatedData: Record<string, any> = {};

        // Map cached company data to expected field names
        if (cachedCompanyData.companyName) {
          const companyNameField = fields.find(
            (f) =>
          ...
          if (companyNameField) {
            const companyName =
              cachedCompanyData.companyName || cachedCompanyData.company_name;
            if (companyName) {
              prePopulatedData[companyNameField.name] = {
                ...
                sourceContext: [
                  {
                    url: `https://${emailContext.domain}`,
                    snippet: "Cached company information",
                  },
                ],
              };
            }
        ...
        if (cachedCompanyData.website) {
          const websiteField = fields.find((f) =>
            f.name.toLowerCase().includes("website"),
          );
          if (websiteField) {
            const website =
              cachedCompanyData.website || cachedCompanyData.website_url;
            if (website) {
              prePopulatedData[websiteField.name] = {
                ...
                sourceContext: [
                  {
                    url: website,
                    snippet: "Cached website information",
                  },
                ],
              };
            }
        ...
        if (cachedCompanyData.description) {
          ...
          if (descField) {
            const description =
              cachedCompanyData.description ||
              cachedCompanyData.company_description;
            if (description) {
              prePopulatedData[descField.name] = {
        ...
        if (cachedCompanyData.industry) {
          const industryField = fields.find((f) =>
            f.name.toLowerCase().includes("industry"),
          );
          if (industryField) {
            const industry = cachedCompanyData.industry;
            if (industry) {
              prePopulatedData[industryField.name] = {
                field: industryField.name,
                value: industry,
                confidence: 0.8,
                source: "cache",
                sourceContext: [
                  {
                    url: `https://${emailContext.domain}`,
                    snippet: "Cached industry information",
                  },
                ],
              };
            }
        ...
        if (cachedCompanyData.employeeCount) {
          ...
          if (employeeField) {
            const employeeCount =
              cachedCompanyData.employeeCount ||
              cachedCompanyData.employee_count;
            if (employeeCount) {
              prePopulatedData[employeeField.name] = {
                ...
                sourceContext: [
                  {
                    url: `https://${emailContext.domain}`,
                    snippet: "Cached employee count information",
                  },
                ],
              };
            }
        ...
        if (cachedCompanyData.location) {
          const locationField = fields.find((f) =>
            f.name.toLowerCase().includes("location"),
          );
          if (locationField) {
            const location = cachedCompanyData.location;
            if (location) {
              prePopulatedData[locationField.name] = {
                ...
                sourceContext: [
                  {
                    url: `https://${emailContext.domain}`,
                    snippet: "Cached location information",
                  },
                ],
              };
            }
        ...
        // Update context with pre-populated data
        Object.assign(context.discoveredData, prePopulatedData);

        // Also add pre-populated data to enrichments so it's returned in results
        Object.assign(enrichments, prePopulatedData);
      }
      console.log(
        `[Orchestrator] Email context: domain=${emailContext.domain}, company=${emailContext.companyNameGuess || "unknown"}`,
      );

      // Step 3: Categorize fields
      const fieldCategories = this.categorizeFields(fields);

      // Step 4: If cached data is available, filter out already-enriched company fields
      if (cachedCompanyData) {
        console.log(
          `[Orchestrator] Filtering out already-enriched company fields`,
        );

        // Remove company fields that are already cached
        const companyFieldNames = Object.keys(context.discoveredData);
        fieldCategories.discovery = fieldCategories.discovery.filter(
          (f) => !companyFieldNames.includes(f.name),
        );
        fieldCategories.profile = fieldCategories.profile.filter(
          (f) => !companyFieldNames.includes(f.name),
        );
        fieldCategories.metrics = fieldCategories.metrics.filter(
          (f) => !companyFieldNames.includes(f.name),
        );
        fieldCategories.funding = fieldCategories.funding.filter(
          (f) => !companyFieldNames.includes(f.name),
        );
        fieldCategories.techStack = fieldCategories.techStack.filter(
          (f) => !companyFieldNames.includes(f.name),
        );
        fieldCategories.other = fieldCategories.other.filter(
          (f) => !companyFieldNames.includes(f.name),
        );

        console.log(
          `[Orchestrator] Remaining fields after filtering: discovery=${fieldCategories.discovery.length}, profile=${fieldCategories.profile.length}, metrics=${fieldCategories.metrics.length}, funding=${fieldCategories.funding.length}, techStack=${fieldCategories.techStack.length}, other=${fieldCategories.other.length}`,
        );

        // Debug log to check feature flag status
        console.log(
          `[Orchestrator] Feature flag status - FEATURE_CONTACT_ONLY_AFTER_FIRST_SCRAPE: ${FIRE_ENRICH_CONFIG.FEATURES.FEATURE_CONTACT_ONLY_AFTER_FIRST_SCRAPE}`,
        );

        // FEATURE_CONTACT_ONLY_AFTER_FIRST_SCRAPE: Skip company-category agents on warm cache
        if (
          FIRE_ENRICH_CONFIG.FEATURES.FEATURE_CONTACT_ONLY_AFTER_FIRST_SCRAPE
        ) {
          console.log(
            `[Orchestrator] FEATURE_CONTACT_ONLY_AFTER_FIRST_SCRAPE enabled - filtering to contact-only fields`,
          );

          // Define person/contact-related field patterns (to be kept)
          const isPersonField = (field: EnrichmentField): boolean => {
            const name = field.name.toLowerCase();
            const desc = field.description.toLowerCase();

            // Person/contact fields: name, surname, role, title, position, linkedin, phone, email
            const personFieldPatterns = [
              "name",
              "surname",
              "lastname",
              "firstname",
            ...
            return personFieldPatterns.some(
              (pattern) => name.includes(pattern) || desc.includes(pattern),
            );
          };

          // Log which fields are being kept for person enrichment
          const personFields = [
            ...fieldCategories.discovery.filter(isPersonField),
            ...fieldCategories.profile.filter(isPersonField),
            ...fieldCategories.metrics.filter(isPersonField),
            ...fieldCategories.funding.filter(isPersonField),
            ...fieldCategories.techStack.filter(isPersonField),
            ...fieldCategories.other.filter(isPersonField),
          ];

          if (personFields.length > 0) {
            console.log(
              `[Orchestrator] Keeping person fields for enrichment: ${personFields.map((f) => f.name).join(", ")}`,
            );
          }

          // Clear ALL company-category fields to skip their agents completely
          // This ensures company agents are never run again for this domain in this batch
          fieldCategories.discovery = [];
          fieldCategories.profile = [];
          fieldCategories.metrics = [];
          fieldCategories.funding = [];
          fieldCategories.techStack = [];
          fieldCategories.other = fieldCategories.other.filter(isPersonField);

          console.log(
            `[Orchestrator] After contact-only filtering: discovery=${fieldCategories.discovery.length}, profile=${fieldCategories.profile.length}, metrics=${fieldCategories.metrics.length}, funding=${fieldCategories.funding.length}, techStack=${fieldCategories.techStack.length}, other=${fieldCategories.other.length}`,
          );
        }
      }
      console.log(
        `[Orchestrator] Field categories: discovery=${fieldCategories.discovery.length}, profile=${fieldCategories.profile.length}, metrics=${fieldCategories.metrics.length}, funding=${fieldCategories.funding.length}, techStack=${fieldCategories.techStack.length}, other=${fieldCategories.other.length}`,
      );

      // Log which agents will be used
      const agentsToUse = [];
      if (fieldCategories.discovery.length > 0)
        agentsToUse.push("discovery-agent");
      if (fieldCategories.profile.length > 0)
        agentsToUse.push("company-profile-agent");
      ...
      if (fieldCategories.techStack.length > 0)
        agentsToUse.push("tech-stack-agent");
      if (fieldCategories.other.length > 0) agentsToUse.push("general-agent");

      console.log(
        `[Orchestrator] Agents to be used: ${agentsToUse.join(", ")}`,
      );
      console.log(
        `[Orchestrator] Agent execution order: ${agentsToUse.join(" → ")}`,
      );

      // Send initial agent progress
      if (onAgentProgress) {
        onAgentProgress(
          `Planning enrichment strategy for ${emailContext.companyNameGuess || emailContext.domain}`,
          "info",
        );
        onAgentProgress(
          `Agent pipeline: ${agentsToUse.map((a) => a.replace("-agent", "").replace("-", " ")).join(" → ")}`,
          "info",
        );
      }

      // Step 3: Progressive enrichment
      // enrichments and context are already initialized above

      // Discovery phase (company identity)
      if (fieldCategories.discovery.length > 0) {
        console.log(
          `[Orchestrator] Activating DISCOVERY-AGENT for fields: ${fieldCategories.discovery.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(
            `Discovery Agent: Identifying company from ${emailContext.domain}`,
            "agent",
          );
          onAgentProgress(
            `Target fields: ${fieldCategories.discovery.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[Orchestrator] DISCOVERY-AGENT completed, found ${Object.keys(discoveryResults).length} values`,
        );
        if (onAgentProgress && Object.keys(discoveryResults).length > 0) {
          onAgentProgress(
            `Discovery complete: Found ${Object.keys(discoveryResults).length} fields`,
            "success",
          );
        }
        ...
        const companyNameField = Object.keys(discoveryResults).find(
          (key) =>
        ...
        if (companyNameField && discoveryResults[companyNameField]) {
          ...
          const companyNameResult = discoveryResults[companyNameField] as
            | { value?: unknown }
            | unknown;
          const companyNameValue =
            companyNameResult &&
            typeof companyNameResult === "object" &&
            "value" in companyNameResult
              ? companyNameResult.value
              : companyNameResult;
          (context as OrchestrationContext).companyName =
            companyNameValue as string;
          console.log(
            `[Orchestrator] Updated context with company name: ${(context as OrchestrationContext).companyName}`,
          );
        }
      ...
      if (fieldCategories.profile.length > 0) {
        console.log(
          `[Orchestrator] Activating COMPANY-PROFILE-AGENT for fields: ${fieldCategories.profile.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(`Profile Agent: Gathering company details`, "agent");
          onAgentProgress(
            `Target fields: ${fieldCategories.profile.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[Orchestrator] COMPANY-PROFILE-AGENT completed, found ${Object.keys(profileResults).length} values`,
        );
        if (onAgentProgress && Object.keys(profileResults).length > 0) {
          onAgentProgress(
            `Profile complete: Found ${Object.keys(profileResults).length} fields`,
            "success",
          );
        }
      ...
      if (fieldCategories.metrics.length > 0) {
        console.log(
          `[Orchestrator] Activating METRICS-AGENT for fields: ${fieldCategories.metrics.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(`Metrics Agent: Analyzing company metrics`, "agent");
          onAgentProgress(
            `Target fields: ${fieldCategories.metrics.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[Orchestrator] METRICS-AGENT completed, found ${Object.keys(metricsResults).length} values`,
        );
        if (onAgentProgress && Object.keys(metricsResults).length > 0) {
          onAgentProgress(
            `Metrics complete: Found ${Object.keys(metricsResults).length} fields`,
            "success",
          );
        }
      ...
      if (fieldCategories.funding.length > 0) {
        console.log(
          `[Orchestrator] Activating FUNDING-AGENT for fields: ${fieldCategories.funding.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(
            `Funding Agent: Researching investment data`,
            "agent",
          );
          onAgentProgress(
            `Target fields: ${fieldCategories.funding.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[Orchestrator] FUNDING-AGENT completed, found ${Object.keys(fundingResults).length} values`,
        );
        if (onAgentProgress && Object.keys(fundingResults).length > 0) {
          onAgentProgress(
            `Funding complete: Found ${Object.keys(fundingResults).length} fields`,
            "success",
          );
        }
      ...
      if (fieldCategories.techStack.length > 0) {
        console.log(
          `[Orchestrator] Activating TECH-STACK-AGENT for fields: ${fieldCategories.techStack.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(`Tech Stack Agent: Detecting technologies`, "agent");
          onAgentProgress(
            `Target fields: ${fieldCategories.techStack.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[Orchestrator] TECH-STACK-AGENT completed, found ${Object.keys(techStackResults).length} values`,
        );
        if (onAgentProgress && Object.keys(techStackResults).length > 0) {
          onAgentProgress(
            `Tech Stack complete: Found ${Object.keys(techStackResults).length} fields`,
            "success",
          );
        }
      ...
      if (fieldCategories.other.length > 0) {
        console.log(
          `[Orchestrator] Activating GENERAL-AGENT for fields: ${fieldCategories.other.map((f) => f.name).join(", ")}`,
        );
        if (onAgentProgress) {
          onAgentProgress(
            `General Agent: Extracting custom information`,
            "agent",
          );
          onAgentProgress(
            `Target fields: ${fieldCategories.other.map((f) => f.name).join(", ")}`,
            "info",
          );
        }
        ...
        console.log(
          `[ORCHESTRATOR] GENERAL-AGENT completed, found ${Object.keys(generalResults).length} values`,
        );
        if (onAgentProgress && Object.keys(generalResults).length > 0) {
          onAgentProgress(
            `General complete: Found ${Object.keys(generalResults).length} fields`,
            "success",
          );
        }
      ...
      const enrichmentResults = this.formatEnrichmentResults(
        enrichments,
        fields,
      );
      ...
      const missingFields = fields
        .filter((f) => !enrichmentResults[f.name]?.value)
        .map((f) => f.name);

      console.log(`[Orchestrator] ====== ENRICHMENT SUMMARY ======`);
      console.log(`[Orchestrator] Email: ${email}`);
      console.log(
        `[Orchestrator] Successfully enriched: ${enrichedFields.length}/${fields.length} fields`,
      );
      if (enrichedFields.length > 0) {
        console.log(
          `[Orchestrator] Enriched fields: ${enrichedFields.join(", ")}`,
        );
      }
      if (missingFields.length > 0) {
        console.log(
          `[Orchestrator] Missing fields: ${missingFields.join(", ")}`,
        );
      }
  ...
  private categorizeFields(fields: EnrichmentField[]) {
    console.log(
      `[Orchestrator] Categorizing ${fields.length} fields for agent assignment...`,
    );
    ...
    for (const field of fields) {
      ...
      if (
        (name.includes("company") && name.includes("name")) ||
        ...
        (name.includes("employee") && name !== "employee_contacts") ||
        name.includes("revenue") ||
        name.includes("size")
      ) {
        categories.metrics.push(field);
      } else if (name === "employee_contacts") {
        ...
        (name.includes("tech") && name.includes("stack")) ||
  ...
  private async runDiscoveryPhase(
    ...
    console.log(
      `[AGENT-DISCOVERY] Fields to discover: ${fields.map((f) => f.name).join(", ")}`,
    );
    ...
    if (ctxEmailContext.companyDomain) {
      ...
      console.log(
        `[AGENT-DISCOVERY] Attempting direct website scrape: ${websiteUrl}`,
      );
      if (onAgentProgress) {
        onAgentProgress(
          `Attempting to access ${ctxEmailContext.companyDomain} directly...`,
          "info",
        );
      }
      try {
        ...
        if (
          ...
          this.isValidCompanyWebsite({
            markdown: scraped.data.markdown,
            metadata: scraped.data as Record<string, unknown>,
          })
        ) {
          console.log(
            `[AGENT-DISCOVERY] Website scrape successful, content length: ${scraped.data.markdown?.length || 0}`,
          );
          if (onAgentProgress) {
            onAgentProgress(
              `Successfully accessed company website (${scraped.data.markdown?.length || 0} chars)`,
              "success",
            );
          ...
          if (companyNameField) {
            ...
            if (companyName) {
              console.log(
                `[AGENT-DISCOVERY] Found company name: ${String(companyName)}`,
              );
              if (onAgentProgress) {
                onAgentProgress(
                  `Extracted company name: ${String(companyName)}`,
                  "success",
                );
              }
          ...
          if (descField) {
            ...
            if (description) {
              if (onAgentProgress) {
                onAgentProgress(
                  `Extracted company description (${description.length} chars)`,
                  "success",
                );
              }
        ...
        console.log(
          `[AGENT-DISCOVERY] Direct website access failed: ${errorMessage}`,
        );
        console.log("[AGENT-DISCOVERY] Activating fallback strategy...");
        if (onAgentProgress) {
          onAgentProgress(
            `Direct website access failed: ${errorMessage.substring(0, 100)}`,
            "warning",
          );
      ...
      console.log(
        "[AGENT-DISCOVERY] No company domain available, skipping direct scrape",
      );
    }
    ...
    if (missingFields.length > 0) {
      console.log(
        `[AGENT-DISCOVERY] Missing fields after direct scrape: ${missingFields.map((f) => f.name).join(", ")}`,
      );
      ...
      if (ctxEmailContext.companyDomain) {
        searchQueries.push(
          `"${ctxEmailContext.companyDomain}" company official website`,
        );
      ...
      if (ctxEmailContext.companyNameGuess) {
        searchQueries.push(
          `"${ctxEmailContext.companyNameGuess}" company official website`,
        );
      }
      ...
      if (ctxEmailContext.domain) {
        searchQueries.push(
          `email domain ${ctxEmailContext.domain} company information`,
        );
      }

      console.log(
        `[AGENT-DISCOVERY] Search queries to try: ${searchQueries.length}`,
      );
      if (onAgentProgress) {
        onAgentProgress(
          `Prepared ${searchQueries.length} search queries for fallback`,
          "info",
        );
      }
      ...
      for (const query of searchQueries) {
        ...
        try {
          ...
          if (onAgentProgress) {
            onAgentProgress(
              `Search ${searchQueries.indexOf(query) + 1}/${searchQueries.length}: ${query.substring(0, 60)}...`,
              "info",
            );
          }
          ...
          if (searchResults && searchResults.length > 0) {
            console.log(
              `[AGENT-DISCOVERY] Found ${searchResults.length} results for query`,
            );
            if (onAgentProgress) {
              onAgentProgress(
            ...
            allSearchResults = allSearchResults.concat(
              searchResults as DiscoverySearchResult[],
            );
          }
        } catch (searchError) {
          console.log(
            `[AGENT-DISCOVERY] Search failed for query "${query}": ${searchError}`,
          );
        }
      }
      ...
      console.log(
        `[AGENT-DISCOVERY] Total unique search results: ${uniqueResults.length}`,
      );
      if (onAgentProgress && uniqueResults.length > 0) {
        onAgentProgress(
          `Processing ${uniqueResults.length} unique search results...`,
          "info",
        );
      }
      ...
      if (uniqueResults.length > 0) {
        ...
        const validResults = uniqueResults.filter((result) => {
          ...
          for (const indicator of parkingIndicators) {
            if (
              ...
              console.log(
                `[AGENT-DISCOVERY] Filtering out domain parking result: ${result.url}`,
              );
              return false;
            }
          }
        ...
        console.log(
          `[AGENT-DISCOVERY] Valid search results after filtering: ${validResults.length}`,
        );
        if (onAgentProgress) {
          onAgentProgress(
            `Filtered to ${validResults.length} valid results`,
            validResults.length > 0 ? "success" : "warning",
          );
        }
        ...
        if (validResults.length > 0) {
          ...
          if (onAgentProgress && Object.keys(extractedData).length > 0) {
            onAgentProgress(
              `Extracted ${Object.keys(extractedData).length} fields from search results`,
              "success",
            );
          }
        } else {
          console.log(
            "[AGENT-DISCOVERY] No valid search results after filtering",
          );
          if (onAgentProgress) {
        ...
        console.log(
          "[AGENT-DISCOVERY] No search results found, using domain-based fallback",
        );
        if (onAgentProgress) {
          onAgentProgress(
            `No search results found, using domain-based inference`,
            "warning",
          );
        }
        // Last resort: use domain-based inference
        const fallbackData = this.inferFromDomain(
          ctxEmailContext,
          missingFields,
        );
        ...
        if (onAgentProgress && Object.keys(fallbackData).length > 0) {
          onAgentProgress(
            `Inferred ${Object.keys(fallbackData).length} fields from domain`,
            "info",
          );
        }
      }
  ...
  private async runProfilePhase(
    ...
    const ctxDiscoveredData = context["discoveredData"] as Record<
      string,
      unknown
    >;
    const companyNameField = Object.keys(ctxDiscoveredData).find(
      (key) =>
        key.toLowerCase().includes("company") &&
        key.toLowerCase().includes("name"),
    );
    ...
    const ctxEmailContext = context["emailContext"] as EmailContext;
    const fieldValue = ctxDiscoveredData[companyNameField || ""] as
      | { value?: unknown }
      | unknown;
    const companyName =
      ctxCompanyName ||
    ...
    console.log(
      `[AGENT-PROFILE] Fields to enrich: ${fields.map((f) => f.name).join(", ")}`,
    );
    if (onAgentProgress) {
      onAgentProgress(
        `Using company name: ${companyName || "Unknown"}`,
        "info",
      );
    }
    ...
    if (!companyName) {
      console.log(
        "[AGENT-PROFILE] No company name available, skipping profile phase",
      );
      return {};
    }
    ...
    if (onAgentProgress) {
      onAgentProgress(
        `Search query: ${searchQuery.substring(0, 100)}...`,
        "info",
      );
    ...
    if (onAgentProgress) {
      if (searchResults.length > 0) {
        onAgentProgress(
          `Found ${searchResults.length} sources with profile data`,
          "success",
        );
        onAgentProgress(
          `Starting corroborated extraction for fields: ${fields.map((f) => f.name).join(", ")}`,
          "info",
        );
      } else {
    ...
    if (companyName && typeof companyName === "string")
      enrichmentContext.companyName = companyName;
    if (ctxEmailContext?.companyDomain)
      enrichmentContext.targetDomain = ctxEmailContext.companyDomain;
    ...
    for (const [fieldName, enrichment] of Object.entries(enrichmentResults)) {
      if (enrichment && enrichment.value) {
        ...
        if (!enrichment.source) {
          enrichment.source = filteredResults
            .slice(0, 2)
            .map((r) => r.url)
            .join(", ");
        }
        ...
        if (enrichment.sourceContext && enrichment.sourceContext.length > 0) {
          ...
          const hasValidUrls = enrichment.sourceContext.some(
            (ctx) => ctx.url && ctx.url !== "extracted",
          );
          if (!hasValidUrls) {
            ...
            if (existingQuote) {
              ...
              const matchingSource = filteredResults.find((r) => {
                ...
                return content.includes(
                  existingQuote.toLowerCase().substring(0, 50),
                );
              });
              ...
              if (matchingSource) {
                ...
                enrichment.sourceContext = filteredResults
                  .slice(0, 1)
                  .map((r) => ({
                    url: r.url,
                    snippet: existingQuote,
          ...
          const { findRelevantSnippet } = await import(
            "../utils/source-context"
          );
          console.log(
            `[SOURCE-CONTEXT] Using fallback snippet extraction for ${fieldName}`,
          );
          ...
          enrichment.sourceContext = filteredResults
            .map((r) => {
              ...
              if (!snippet) {
                console.log(
                  `[SOURCE-CONTEXT] No snippet found for ${fieldName} value "${enrichment.value}" in ${r.url}`,
                );
              }
            ...
            .filter((ctx) => {
              ...
              if (!hasSnippet) {
                console.log(
                  `[SOURCE-CONTEXT] Filtering out empty snippet for ${fieldName} from ${ctx.url}`,
                );
              }
              return hasSnippet;
            })
          ...
          console.log(
            `[SOURCE-CONTEXT] Final source context for ${fieldName}: ${enrichment.sourceContext.length} sources`,
          );
        }
      }
    }
  ...
  private async runMetricsPhase(
    ...
    console.log(
      `[AGENT-METRICS] Fields to enrich: ${fields.map((f) => f.name).join(", ")}`,
    );

    if (!companyName) {
      console.log(
        "[AGENT-METRICS] No company name available, skipping metrics phase",
      );
      return {};
    }
    ...
    if (onAgentProgress) {
      onAgentProgress(
        `Found ${searchResults.length} sources with metrics data`,
        searchResults.length > 0 ? "success" : "warning",
      );
    }

    // Extract metrics with OpenAI
    const combinedContent = this.trimSearchResultsContent(
      searchResults,
      250000,
    );
    ...
    if (onAgentProgress && searchResults.length > 0) {
      onAgentProgress(
        `Extracting metrics from ${searchResults.length} sources...`,
        "info",
      );
    }
  ...
  private async runFundingPhase(
    ...
    console.log(
      `[AGENT-FUNDING] Fields to enrich: ${fields.map((f) => f.name).join(", ")}`,
    );

    if (!companyName) {
      console.log(
        "[AGENT-FUNDING] No company name available, skipping funding phase",
      );
      return {};
    }
    ...
    if (onAgentProgress) {
      onAgentProgress(
        `Found ${searchResults.length} sources with funding data`,
        searchResults.length > 0 ? "success" : "warning",
      );
    }
  ...
  private async runTechStackPhase(
    ...
    console.log(
      `[AGENT-TECH-STACK] Company name: ${companyName || "Not found"}`,
    );
    console.log(
      `[AGENT-TECH-STACK] Company domain: ${companyDomain || "Not found"}`,
    );
    console.log(
      `[AGENT-TECH-STACK] Fields to enrich: ${fields.map((f) => f.name).join(", ")}`,
    );

    if (onAgentProgress) {
      onAgentProgress(
        `Using company: ${companyName || companyDomain || "Unknown"}`,
        "info",
      );
    }
    ...
    if (!companyName && !companyDomain) {
      console.log(
        "[AGENT-TECH-STACK] No company name or domain available, skipping tech stack phase",
      );
      return {};
    }
    ...
    try {
      ...
      console.log(
        `[AGENT-TECH-STACK] Found ${githubResults.length} valid GitHub results`,
      );
      if (githubResults.length > 0) {
        console.log(
          `[AGENT-TECH-STACK] GitHub URLs: ${githubResults.map((r) => r.url).join(", ")}`,
        );
      }
    } catch (error) {
    ...
    if (companyDomain) {
      try {
        console.log(
          `[AGENT-TECH-STACK] Fetching HTML from company website for analysis`,
        );
        const websiteData = await this.firecrawl.scrapeUrl(
          `https://${companyDomain}`,
        );
        if (websiteData.data && websiteData.data.html) {
          websiteHtml = websiteData.data.html;
          console.log(
            `[AGENT-TECH-STACK] HTML fetched, length: ${websiteHtml.length}`,
          );

          // Analyze HTML for technology indicators
          detectedTechnologies = this.analyzeTechStackFromHtml(websiteHtml);
          console.log(
            `[AGENT-TECH-STACK] Detected technologies from HTML: ${detectedTechnologies.join(", ")}`,
          );
        }
      } catch (error) {
    ...
    console.log(
      `[AGENT-TECH-STACK] Tech stack search query: ${techSearchQuery}`,
    );
    ...
    console.log(
      `[AGENT-TECH-STACK] Found ${techResults.length} tech stack results`,
    );

    if (onAgentProgress) {
      onAgentProgress(
        `Found ${techResults.length} sources with tech stack data`,
        techResults.length > 0 ? "success" : "info",
      );
    }
    ...
    let combinedContent = this.trimSearchResultsContent(
      allSearchResults,
      200000,
    ); // Smaller limit for tech stack
    ...
    if (
      !hasValidGithubContent &&
      !hasValidTechContent &&
      detectedTechnologies.length === 0
    ) {
      console.log(
        "[AGENT-TECH-STACK] No valid tech stack information found, returning empty results",
      );
      return {};
    }
    ...
    if (githubResults.length > 0) {
      enrichmentContext.validGithubUrls = githubResults
        .map((r) => r.url)
        .join(", ");
    }
    ...
    for (const [fieldName, enrichment] of Object.entries(enrichmentResults)) {
      if (enrichment && enrichment.value) {
        ...
        if (
          enrichment.sourceContext &&
          Array.isArray(enrichment.sourceContext)
        ) {
          enrichment.sourceContext = enrichment.sourceContext.filter((ctx) => {
            ...
            if (ctx.url.includes("github.com")) {
              const isValidGithub = githubResults.some(
                (r) => r.url === ctx.url,
              );
              if (!isValidGithub) {
                console.log(
                  `[AGENT-TECH-STACK] Removing hallucinated GitHub URL: ${ctx.url}`,
                );
                return false;
              }
            }
  ...
  private async runGeneralPhase(
    ...
    console.log(
      `[AGENT-GENERAL] Company domain: ${companyDomain || "Not found"}`,
    );
    console.log(
      `[AGENT-GENERAL] Fields to enrich: ${fields.map((f) => f.name).join(", ")}`,
    );
    ...
    if (!companyName && !companyDomain) {
      console.log(
        "[AGENT-GENERAL] No company name or domain available, skipping general phase",
      );
      return {};
    }

    // Build targeted search queries for the requested fields
    const searchQueries = this.buildGeneralSearchQueries(
      fields,
    ...
    if (onAgentProgress && searchQueries.length > 0) {
      onAgentProgress(
        `Prepared ${searchQueries.length} search queries for custom fields`,
        "info",
      );
    }
    ...
    for (let i = 0; i < searchQueries.length; i++) {
      ...
      try {
        ...
        if (onAgentProgress) {
          onAgentProgress(
            `Search ${i + 1}/${searchQueries.length}: ${query.substring(0, 60)}...`,
            "info",
          );
        }
        const searchResults = await this.firecrawl.search(query, {
    ...
    if (companyDomain && this.hasExecutiveFields(fields)) {
      if (onAgentProgress) {
        onAgentProgress(
          `Checking company website for executive information...`,
          "info",
        );
      }
      ...
      for (let i = 0; i < executiveUrls.length; i++) {
        ...
        try {
          ...
          if (scraped.data && scraped.data.markdown) {
            ...
            if (onAgentProgress) {
              onAgentProgress(
                `Found executive information on ${url.split("/").pop()} page`,
                "success",
              );
            }
    ...
    console.log(
      `[AGENT-GENERAL] Total unique results: ${uniqueResults.length}`,
    );
    ...
    const foundFields = Object.keys(enrichmentResults).filter(
      (k) => enrichmentResults[k]?.value,
    );
    if (onAgentProgress && foundFields.length > 0) {
      onAgentProgress(
        `Successfully extracted ${foundFields.length} custom fields`,
        "success",
      );
    }
  ...
  private buildGeneralSearchQueries(
    fields: EnrichmentField[],
    companyName?: string,
    ...
    if (executiveFields.length > 0) {
      const titles = executiveFields
        .map((f) => this.extractTitle(f))
        .filter(Boolean);

      if (companyName) {
        queries.push(
          `"${String(companyName)}" leadership team executives ${titles.join(" ")}`,
        );
        queries.push(
          `"${String(companyName)}" CEO CTO CFO founders management`,
        );
      }
    ...
    for (const field of otherFields) {
      // Special handling for employee_contacts field - use targeted search terms
      if (field.name === "employee_contacts") {
        if (companyName) {
          queries.push(
            `"${String(companyName)}" contact page team employees staff leadership`,
          );
          queries.push(
            `"${String(companyName)}" about us team members directory`,
          );
          queries.push(
            `"${String(companyName)}" leadership team management employees contact`,
          );
        }
        if (companyDomain) {
          queries.push(`site:${companyDomain} contact`);
          queries.push(`site:${companyDomain} team`);
          queries.push(`site:${companyDomain} about`);
          queries.push(`site:${companyDomain} leadership`);
          queries.push(`site:${companyDomain} employees`);
          queries.push(`site:${companyDomain} staff`);
        }
      } else if (field.name === "business_partners") {
        // Enhanced search for business partners
        if (companyName) {
          queries.push(
            `"${String(companyName)}" partnerships integrations clients collaborators`,
          );
          queries.push(
            `"${String(companyName)}" business partners technology alliances`,
          );
        }
        if (companyDomain) {
          queries.push(`site:${companyDomain} partners`);
          queries.push(`site:${companyDomain} integrations`);
          queries.push(`site:${companyDomain} clients`);
        }
      } else {
  ...
  private isExecutiveField(field: EnrichmentField): boolean {
    ...
    return executiveTitles.some(
      (title) => name.includes(title) || desc.includes(title),
    );
  }
    ...
    if (field.description) {
      ...
      const keyPhrases = field.description
        ...
        .filter(
          (word) =>
            word.length > 3 &&
            !["this", "that", "what", "when", "where", "which"].includes(word),
        );
  ...
  private analyzeTechStackFromHtml(html: string): string[] {
    ...
    if (html.match(/class=["'][^"']*\bmui-[^"'\s]+/))
      technologies.add("Material-UI");
    if (html.match(/class=["'][^"']*\bant-[^"'\s]+/))
      technologies.add("Ant Design");
    if (html.match(/class=["'][^"']*\bchakra-[^"'\s]+/))
      technologies.add("Chakra UI");
  ...
  private formatEnrichmentResults(
    ...
    for (const field of fields) {
      ...
      // If we only have a raw value (shouldn't happen anymore, but keep as safety)
        ...
        console.warn(
          `[ORCHESTRATOR] Raw value found for field ${field.name}, this shouldn't happen`,
        );
  ...
  private isValidCompanyWebsite(scraped: {
    markdown?: string;
    ...
    for (const indicator of invalidIndicators) {
      if (markdown.includes(indicator) || title.includes(indicator)) {
        console.log(
          `[ORCHESTRATOR] Detected invalid website indicator: "${indicator}"`,
        );
        return false;
      }
    }
    ...
    if (markdown.length < 200) {
      console.log(
        `[ORCHESTRATOR] Content too short (${markdown.length} chars), likely placeholder`,
      );
      return false;
    }
    ...
    if (!hasLegitimateContent) {
      console.log(
        `[ORCHESTRATOR] No legitimate company content indicators found`,
      );
      return false;
    }
  ...
  private extractCompanyName(scraped: {
    markdown?: string;
    ...
    if (!this.isValidCompanyWebsite(scraped)) {
      console.log(
        "[ORCHESTRATOR] Invalid company website detected, skipping extraction",
      );
      return null;
    }
    ...
    console.log(
      `[ORCHESTRATOR] Extracting company name for domain: ${urlDomain}`,
    );
    ...
    if (knownCompanies[baseDomain]) {
      console.log(
        `[ORCHESTRATOR] Found known company: ${knownCompanies[baseDomain]}`,
      );
      return knownCompanies[baseDomain];
    }

    // Look for og:site_name meta tag first (most reliable)
    const ogSiteNameMatch = markdown.match(
    ...
    if (ogSiteNameMatch && ogSiteNameMatch[1]) {
      ...
      if (siteName && siteName.length > 2) {
        console.log(
          `[ORCHESTRATOR] Found company name in og:site_name: ${siteName}`,
        );
        return siteName;
      }
    }
    ...
    for (const pattern of companyPatterns) {
      ...
      if (match && match[1]) {
        ...
        if (
          nameLower.includes(baseDomain) ||
          baseDomain.includes(nameLower.substring(0, 4))
        ) {
    ...
    if (metadata.title && typeof metadata.title === "string") {
      const cleaned = metadata.title
        .replace(
          /\s*[\||-]\s*(?:Official\s*)?(?:Website|Site|Home|Page)?\s*$/gi,
          "",
        )
        .replace(/\s*[\||-]\s*[^|]+$/i, "")
        .replace(/\s*:\s*[^:]+$/i, "")
      ...
      if (cleaned && cleaned.length > 2) {
        ...
        if (
          cleanedLower.includes(baseDomain) ||
          baseDomain.includes(cleanedLower.substring(0, 4))
        ) {
  ...
  private extractDescription(scraped: {
    markdown?: string;
    ...
    if (!this.isValidCompanyWebsite(scraped)) {
      console.log(
        "[ORCHESTRATOR] Invalid company website detected, skipping description extraction",
      );
      return null;
    }
  ...
  private async extractFromSearchResults(
    ...
    if (onAgentProgress) {
      onAgentProgress(
        `Analyzing content from ${searchResults.length} sources...`,
        "info",
      );
    }
...
IMPORTANT: Only extract information that is clearly about the company associated with the email domain ${emailContext.domain}.
    ...
    try {
      if (onAgentProgress) {
        onAgentProgress(
          `Using AI to extract ${fields.map((f) => f.name).join(", ")}...`,
          "info",
        );
      }
      ...
      if (onAgentProgress && foundFields.length > 0) {
        onAgentProgress(
          `Successfully extracted ${foundFields.length} fields from search results`,
          "success",
        );
      }
      ...
      for (const [, enrichment] of Object.entries(enrichmentResults)) {
        if (enrichment && enrichment.value) {
          enrichment.source = searchResults
            .slice(0, 2)
            .map((r) => r.url)
            .join(", ");
      ...
      console.error(
        "[AGENT-DISCOVERY] Failed to extract from search results:",
        error,
      );
      return {};
    }
  ...
  private inferFromDomain(
    ...
    console.log(
      "[AGENT-DISCOVERY] Using domain-based inference as last resort",
    );
    const results: Record<string, unknown> = {};
    ...
    console.log(
      `[AGENT-DISCOVERY] Inferred ${Object.keys(results).length} fields from domain`,
    );
    return results;
  }
  ...
  private trimSearchResultsContent(
    ...
    console.log(
      `[ORCHESTRATOR] Content size ${totalSize} exceeds limit ${maxTotalChars}, trimming...`,
    );

    // Calculate chars per result (ensure at least 1000 chars per result)
    const charsPerResult = Math.max(
      1000,
      Math.floor(maxTotalChars / searchResults.length),
    );
